...
Per afegir nous tallers o events cal editar el fitxer `src/data/content.ts`. Els dos tallers que tinguin places més propers es mostraràn a la pàgina principal.

### 1. Afegir un nou event al calendari

1. Anar al fitxer [src/data/content.ts](src/data/content.ts), editar el fitxer clicant ✎ i modificar `events`.
...
4. Anar al fitxer [src/data/content.ts](src/data/content.ts), editar el fitxer clicant ✎ i modificar `tallers`. Veuràs que és una llista amb l'estrctura `{tallerID: {...dades del taller, tallerId2: {...dades del segon taller}}` és important que aquest `tallerId` sigui únic, i l'hauras d'utilitzar després quan afegeixis un event.
5. Un cop afegit el nou taller (pots consultar l'estructura a la part superior del mateix fitxer), clicar <b style="color: green">Commit changes...</b>. Si cliques 'Commit directly to `main` branch' els canvis s'aplicarán directament. També pots 'create a new branch and start a pull requests', lo qual et permetrà veure una previsualització dels canvis abans de publicar. Un cop contenta amb els canvis, clica <b style="color: green">Merge Pull Request</b>.

### 3. Modificar algun text

Tots els textos de la pàgina es troben a la carpeta [src/data/translations.ts](src/data/translations.ts). Per modificar-los, només cal editar el fitxer corresponent.
